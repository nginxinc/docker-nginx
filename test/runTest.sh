...
docker run -d --rm --name nginx-test -v "$(pwd)/conf.d:/etc/nginx/conf.d" -v "$(pwd)/modsec:/etc/nginx/modsec" -v "$(pwd)/nginx.conf:/etc/nginx/nginx.conf" -p 9080:80 -p 9085:8085 "$image"
sleep 10
